...
After going through each row, ```Name```, ```Color 1```, and ```Color 2``` values are taken and used to construct the ```colorsOfType``` dictionary. This loop works the same as the loop for making the ```variations``` dictionary, except if cOne != cTwo, it is counted twice. Then the key:value pairs in the dictionary are printed out.
